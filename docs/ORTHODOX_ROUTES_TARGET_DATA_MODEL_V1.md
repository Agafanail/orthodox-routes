...
- **Physical status:** canonical logical design with implemented local Account & Eligibility, provider-independent Phone Verification, contextual-registration, and Core transport/response/agreement foundations, not a complete executable target domain schema, API contract, or deployed domain model. Versioned migrations now materialize `app.account`, `private.account_contact`, legal-document/acceptance metadata, narrow account/eligibility RPCs, the protected phone-attempt/delivery boundary, protected incomplete-action drafts, separate temporary registration profiles, server-only rate buckets, canonical church/service transport references, passenger requests, driver series, date-specific occurrences, responses, immutable conditions, agreements, contact snapshots, and safe agreement history on top of the `app`, `private`, `api`, and `ops` schemas; external providers, recovery, Maps geography, application cutover, and remaining target entities are not implemented
...
Until the explicitly later Maps phase, `private.user_place` contains bounded protected exact and public-area labels but no coordinate, provider, radius, or route claim. This is a compatible non-map Core representation, not a substitute for the approved 1 km stable area, protected coordinates, `driver_route_input`, public corridor, PostGIS indexes, or quality matching. Browser-local prototype records are neither imported nor trusted. Responses, condition snapshots, agreements, capacity, cancellation, restoration, disclosure, and lifecycle anonymization are now implemented through protected functions; application cutover remains subsequent campaign work.
