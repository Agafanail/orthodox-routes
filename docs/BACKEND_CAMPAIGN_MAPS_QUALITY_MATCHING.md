...
| G | Full campaign verification, privacy audit, human UX gate, release | Complete | CI green on `1c94ddb` across all four jobs five consecutive times — `33016589727`, `33016856712`, `33017129980`, `33017555476`, `33017810667` — including the browser regression that had been failing roughly every other run. `047e7a0` then merged the owner's copy review group 2B from `main` and is green on all four jobs as run `33018778122`, with the pull request mergeable. The group 2B amendment moved the map onto the «Где вас забрать?» screen itself; the campaign's separate rule that no public-area field exists is kept beside it, because it states a different fact. The provider credential now answers on every endpoint, address search is verified across four countries on the deployment, matching is verified against live routing, and the privacy boundary is re-verified on the deployed anonymous page. The blank map the owner found on the first walkthrough is fixed and the maps are confirmed drawing. The time rule was then changed twice by the owner and now compares the two stated times directly; CI `33217590482` is green on `d0029ec` across all four jobs, including a clean migration replay from empty. Both migrations are applied on staging. Acceptance testing then found matching silent on the deployment; the cause was one unroutable meeting place, fixed in `7c7b200` and confirmed live. The owner then completed the manual acceptance test and signed it off; see *Closure* below |

**Current continuation:** none. The campaign is closed.

### Closure — 2 September 2026

The owner completed the full manual acceptance test and signed it off: catalog and maps, the Geoapify picker and search, public approximate areas, the Google and Yandex outbound links, the `Подходит` marker, `Подходящие мне`, the detour limit, the time rule, passengers and drivers told apart on the map, privacy before confirmation, exact data disclosed only after mutual confirmation, anonymous and unrelated users kept out, cancellation, republication when the driver withdraws and no republication when the passenger does, and mobile.

**Final audit before merge.** No excluded concept exists in any migration or in the geo library: `pgRouting`, corridor, polyline, stored route geometry, compatibility score and match percentage appear only in documentation stating their absence, in comments saying so, and in the checks that assert it. No secret-shaped value is in the diff. Every public projection — active requests, active occurrences, published churches, quality matches — exposes approximate areas only. The route-worker bridge is granted to `service_role` alone. `api.get_agreement_exact_place` serves only the two participants, only while the agreement is confirmed or later, never once cancelled, and gives the driver only the meeting place selected for that agreement rather than the passenger's other saved places. Matching stays advisory: an unmeasured pair is never a negative claim and the ordinary board is never restricted by it. A leg the provider cannot route is skipped rather than reported as an outage. Republication after a driver's withdrawal restores the existing request and creates none; a passenger's own cancellation restores nothing; a request whose time has passed is never republished.

**Known limitations, unchanged and accepted.** Desktop Chrome on Windows sometimes reports a position hundreds of kilometres away while the same account's mobile browser is correct. That is the browser and operating system answering, not the application: there is no IP fallback and no coordinate substitution anywhere in the code, and the position is used exactly as given, rounded to about a kilometre before it travels anywhere. Google and Yandex remain outbound links; Geoapify remains the only embedded map and geospatial provider for V1. `My Trips` maps stay outside this scope.

**`test:staging-core` was not re-run at closure.** It requires an entirely empty project and truncates accounts and churches to start, so its own guard correctly refuses while the acceptance fixture is present. It passed earlier in the campaign against the deployed schema.
...
### Three findings from the owner's acceptance test — 29 August 2026

**«Рядом со мной» placed the owner in Bosnia.** Not an application defect, and worth stating precisely because the temptation is to patch it. The application has no location source of its own: no address lookup, no address-derived guess, no default coordinate, no provider call, and nothing that could move a point by five hundred kilometres — the only arithmetic applied is a rounding to about a kilometre. The position came from the browser.

It was, however, being asked for badly. The call accepted a cached position up to five minutes old and told the browser that a rough one would do, which is exactly the request that invites a network-derived answer resolved from the connection rather than the device. It now asks for a fresh best-effort fix, and it reads the accuracy the browser itself reports: past twenty-five kilometres of uncertainty it declines to sort and tells the person how vague the answer was, instead of presenting it as fact. Nothing is corrected, substituted, or guessed. If a browser reports a confident wrong position there is nothing honest an application can do about it, and the message points at the browser and system settings where the fix belongs.

**The Yandex route link was malformed.** Yandex documents `rtext` as `lat,lon~lat,lon` — both ends of a route. Half of one is not a route, so Yandex discarded it and opened a generic map wherever it pleased. Building the real thing needs an origin, and this product will not learn where someone is for the sake of a link. The church is opened as a place instead, through the documented `whatshere` form: Yandex names the point and offers its own «Маршрут» beside it, starting from the location the person already granted Yandex. Verified against the deployment — it opens in Catanzaro on the church's own coordinate. Note that `ll`, `pt`, and `whatshere` order longitude first while `rtext` orders latitude first; that ordering is now written once and reused, because reversing it fails silently. Google's link was correct and is untouched.

**The staging church address and coordinate described different places.** It displayed «Via Giuseppe Verdi 12, Torino» while its coordinate resolved to Piazza Castello — and both were in Turin, when the bundled reference data has always placed this parish in Catanzaro, where the owner is. The walkthrough now sits in Catanzaro, and every coordinate in the fixture was read back from the provider before being written, so each stored address describes its own point.

**Ordering can now be judged by eye.** Four published churches stand at roughly 1, 25, 52 and 246 km, and the catalog orders them exactly so, each card carrying its own distance. Two of them — Lamezia Terme and Cosenza — have no page of their own, because the church screen still resolves its slug against the bundled reference data; the fixture summary says so plainly rather than leaving a surprise.

CI `33276620730` green on `4407739` across all four jobs.
