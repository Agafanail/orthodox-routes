...
- **Implementation state:** the version-controlled local Supabase/PostgreSQL foundation and an isolated synthetic-only staging project/HTTPS deployment run the committed Core migrations, passwordless email identity/session boundary, account/consent/eligibility and provider-independent phone-verification foundations, protected contextual registration through explicit publication, server-owned Core transport/response/agreement boundaries, readiness checks, and configured Core application cutover; real external email/SMS delivery, production access, production legal documents, Maps/provider integration, and remaining target domain schema are not configured
