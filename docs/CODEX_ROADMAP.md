...
- Application-owned phone verification with versioned policy, idempotent attempts, transient restricted delivery material, expiry and attempt limits, race-safe uniqueness, authoritative eligibility transition, a targeted service-role-only worker bridge, and a Bird SMS adapter. Bird credentials and real delivery remain outside the repository; no production fake is configured.
...
The isolated synthetic-only Supabase staging project and HTTPS deployment run the committed Core migrations, the configured RPC board, readiness checks, and guarded remote multi-user smoke verification. Staging Auth has an owner-configured Resend SMTP sender and real Gmail delivery is confirmed, with Spam placement tracked separately; the live link/session completion smoke remains pending. Bird is the owner-approved SMS provider and the repository implements the Bird adapter and protected worker bridge, but real SMS delivery remains unverified until the owner-held key is injected. The repository still has no production backend access, push delivery, production Terms/Privacy text, Firebase, Google Maps, Telegram, payments, WhatsApp API, or admin features. The contextual flow is connected from passenger/driver input through verified-email claim, account materialization, adult declaration, phone challenge, Terms acceptance, explicit final review, and idempotent publication. Local browser and remote API/web verification use synthetic legal and eligible-account fixtures; real participation remains correctly gated until current legal content and phone delivery are configured. The configured church transport board is cut over to the server-owned transport, response, agreement, capacity, cancellation, restoration, and disclosure RPCs with no `localStorage` fallback or dual write.
...
- Implement the passwordless email identity/session foundation — completed locally; the isolated staging Auth callback and owner-configured Resend SMTP sender are configured and real Gmail delivery is confirmed. Spam placement is a deliverability issue, the fresh live link/session smoke remains pending, and production email is absent.
...
- Implement the Phone Verification Foundation and provider adapter — database ownership, targeted service-role worker bridge, fail-closed Bird adapter, and application integration are complete locally; the owner-controlled Bird key is not injected and real SMS delivery remains unverified.
...
- Keep real SMS delivery behind owner-controlled billing, sender registration, regional routing, and secret injection. Bird and the `OrthoRoutes` Italy sender are approved for staging; only the server-owned delivery adapter is committed, never the Bird API key.
