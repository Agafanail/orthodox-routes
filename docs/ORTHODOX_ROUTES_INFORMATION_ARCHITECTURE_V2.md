...
5. Указать допустимое отклонение от маршрута в километрах.
6. Указать число свободных мест.
7. Указать, может ли водитель перевозить детей и предоставляет ли детское кресло.
8. Отметить возможность поездки обратно.
9. Добавить короткое примечание.
10. Проверить точные данные и их примерное публичное представление.
11. Войти или создать аккаунт.
12. Подтвердить email и телефон, если нужно.
13. Опубликовать.

Отдельный шаг построения маршрута из формы убран: маршрут рассчитывается системой только во время проверки «по пути» (§6.3) и не является данными объявления.
...
Перед публикацией водитель видит, как публично будет выглядеть примерная область отправления. Точный адрес отправления публично не показывается, а публичной линии маршрута не существует. Точное место отправления открывается пассажиру только после взаимного подтверждения.
...
- одна и та же служба либо совместимые собственные дата и время: расчётное прибытие водителя к храму **с учётом заезда за пассажиром** не раньше чем на час и не позже чем на полчаса от желаемого времени пассажира (поправка от 27 августа 2026 года);
...
- число мест;
- дети и детское кресло;
- поездка обратно;
- примечание;
- проверка точных данных и публичного представления;
- предупреждение о примерной области и примерном коридоре.
...
4. Указывает точное отправление, отклонение, места и детей.
