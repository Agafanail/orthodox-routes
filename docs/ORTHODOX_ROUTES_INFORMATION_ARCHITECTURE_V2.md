...
Карта — **вложенный экран этого же экрана, а не отдельный смысловой шаг.** Путь: «Где вас забрать?» → открыть карту → выбрать место → вернуться на тот же экран с сохранённым местом → при желании добавить до двух альтернатив. Возврат с карты не считается переходом к следующему вопросу формы, а «Подтвердить место» на карте не является действием «Далее».

После возврата выбранный адрес показывается **один раз**, рядом с ним — «Изменить это место», ниже — «Добавить место». Отдельного поля для названия места форма не предлагает: ориентир вроде «у входа в библиотеку» человек пишет в обычном необязательном примечании на экране 4 (§10.1, экран 4). Второе поле, похожее на адрес, рядом с адресом только запутывает.
...
- Место выбирается на полноценном экране карты по правилам §6.6.
