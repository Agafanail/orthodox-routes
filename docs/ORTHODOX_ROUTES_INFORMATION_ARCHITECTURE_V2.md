...
- фотографию или нейтральное стандартное изображение.
...
На телефоне по умолчанию открыт список; переключатель **«Список / Карта»** сохраняет поиск и фильтры.
...
По умолчанию показаны обе категории. Предложения и просьбы различаются текстом, содержанием, формой маркера и основным действием; цвет служит только дополнительным признаком.

На телефоне карточки находятся в одном потоке. На компьютере допустимы две колонки, если порядок чтения и работа с клавиатуры остаются понятными.
...
После времени поездки обе стороны получают один короткий вопрос:
...
- Телефон: список или карта по переключателю.
...
- Компьютер: основной контент слева; действия и компактная карта справа.
...
stateDiagram-v2
    ...
    EmailVerified --> Active: телефон подтверждён, 18+ и Условия приняты
...
Email-проверка сама по себе не даёт права публиковать. Ограничение не отнимает возможность отменить поездку, получить данные и удалить аккаунт.
