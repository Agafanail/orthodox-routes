...
Транспортная доска остаётся представлением по умолчанию. Карта — отдельное пространственное представление той же группы, помогающее сориентироваться тем, кто плохо знает район. Она не заменяет доску и не заменяет будущий качественный подбор поездок.
...
**Примерное расстояние.** Когда местоположение доступно, карточка может показывать короткое округлённое значение вида **«≈ 6 км от вас»**. Оно рассчитывается только до публичной примерной области или до публичного коридора и никогда до скрытого точного места или скрытой геометрии маршрута. Если местоположение недоступно, строка не показывается.
