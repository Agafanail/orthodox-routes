...
- **Последнее утверждённое изменение:** 2 сентября 2026 года — форма предложения водителя из четырёх смысловых экранов утверждена владельцем по итогам поэкранной проверки группы 3 и приведена в соответствие с кампанией Maps & Quality Matching: публичного маршрута водителя нет (§6.3, §11.6), время выезда форма не собирает, требование `PS §9.1` о нём снято (§11.7). Ранее: 28 августа 2026 года — упрощение экранов формы предложения водителя: экран «откуда» разделён на два состояния одного вопроса и освобождён от времени выезда, все оставшиеся изменяемые сведения собраны на экране «детали поездки», экран «проверьте поездку» стал экраном только для просмотра без публичного представления поездки (§11.1, §11.6, §11.7, §31.2, §32.2). Ранее: 27 августа 2026 года — упрощение формы предложения водителя до четырёх смысловых экранов без отдельных экранов вида поездки, маршрута и проверки; объяснение публичности перенесено на экран выбора места (§11, §11.1, §11.6, §11.7, §31.2). Ранее: 22 августа 2026 года — правки владельца по итогам первой поэкранной проверки русской копии: общий поиск каталога (§8.1), строка адреса без видимой подписи действия (§9), одно публичное хронологическое расписание (§9.1), ролевые фильтры и метки доски (§9.2); в тот же день — упрощение формы просьбы пассажира до четырёх смысловых экранов без отдельного экрана проверки (§10.1, §10.3, §10.4, §28.3, §31.2, §32.1)
...
| Примерная область отправления | Публичная область около 1 км, содержащая точное место отправления водителя, но не указывающая его центр |
...
| Смотреть примерные области | Да | Да | Да | Да | Да |
...
- видеть только примерные области;
...
Номер телефона принадлежит аккаунту только после подтверждения. Один подтверждённый номер не может одновременно принадлежать двум аккаунтам; до подтверждения одинаковый введённый номер может временно находиться в нескольких незавершённых регистрациях.
...
| Примерная область отправления водителя | Публично, пока предложение активно | Участникам и публично до закрытия | Не показывается по старой ссылке | Может сохраниться только как обезличенная область в истории |
| Точное место встречи и точное место отправления | Никому, кроме самого автора | Только участникам конкретной договорённости и только выбранное для неё место | Доступ закрывается сразу | Удаляются или необратимо обезличиваются |
| Публичная линия маршрута водителя | Не существует | Не существует | Не существует | Не существует |
...
### 6.3. Место отправления водителя и отсутствие публичного маршрута

Публичного маршрута водителя не существует.

- Место отправления водителя показывается публично только примерной областью по правилам §6.2.
- Публичной линии улиц, публичного коридора направления и публичной геометрии маршрута нет.
- Водитель не обещал ехать по дороге, которую рассчитал внешний сервис, поэтому такая дорога не показывается как маршрут поездки и не сохраняется как обязательство.

Маршрут рассчитывается только внутри проверки «по пути»: он сравнивает обычную дорогу к храму с дорогой через место пассажира и даёт дополнительные километры и примерные дополнительные минуты. Результатом является число, а не публикуемая геометрия.

**Рассчитанная дорога как временная подсказка автору.** Пока водитель создаёт или изменяет предложение, рассчитанная дорога может быть **показана ему самому** на карте формы — как временная зрительная подсказка, помогающая понять выбранное место отправления. Это допущение не отменяет ни одного правила выше:

- дорога **не входит в опубликованное предложение** и не является его данными;
- дорога **никому, кроме автора, не показывается** — ни посетителям, ни пассажиру после договорённости;
- дорога **не сохраняется** как маршрут водителя и как его обязательство;
- водитель **не обязуется ехать именно по ней**: она описывает расчёт, а не обещание.

Подсказка существует только внутри формы и исчезает вместе с ней. Отдельного предупреждения или пояснения рядом с ней в интерфейсе **не показывается**: правило о публичности места сказано на том же экране один раз (§11.6), а второй текст рядом обесценил бы первый.

Повторные публикации из одного и того же места не должны позволять усреднить несколько публичных областей и восстановить точное отправление.
...
- водителю — точное место встречи, выбранное именно для этой договорённости;
- пассажиру — точное место отправления водителя;
- подтверждённый телефон;
- подтверждённый email;
- имя второй стороны.

Остальные точные места пассажира водителю не открываются: договорённость раскрывает только выбранное место. Администратор страницы храма не получает доступ к этим данным на основании управления храмом.
...
После отмены или по истечении утверждённого срока доступ к этим данным закрывается по правилам договорённости, а не остаётся у второй стороны навсегда.

### 6.6. Выбор места

Место выбирается одинаково для места встречи пассажира и места отправления водителя.

**Основной порядок:**

1. человек ищет адрес или название места;
2. выбирает найденный вариант;
3. видит выбранную точку на карте;
4. при необходимости перемещает маркер;
5. подтверждает место.

**Запасной порядок.** Если адресных данных не хватает — а в сёлах и малых населённых пунктах это обычная ситуация — человек может поставить или передвинуть точку вручную. Признанный поставщиком почтовый адрес не является обязательным условием: достаточно правильно подтверждённой точки на карте.

**Только реальное место.** Человек указывает одно реальное место. Отдельного поля публичной области нет; публичное представление создаёт система по правилам §6.2.

**Местоположение устройства.** Разрешение запрашивается только после явного действия **«Показать, где я»**. При открытии карты разрешение не запрашивается, фонового слежения нет.

### 6.7. Сохранённые места

Человек может сохранить выбранное точное место для повторного использования.

- Сохранённое место хранится, пока человек сам его не удалит.
- Сохранённые места можно повторно выбрать при публикации, переименовать и удалить.
- При публикации всегда доступен выбор нового места; сохранённые места не навязываются.
- Сохранённые места видны только их владельцу и не публикуются сами по себе: публичной становится только та примерная область, которая относится к активному объявлению.
...
**Общий поиск** ищет по локализованному названию храма, официальному названию, населённому пункту и стране одной строкой. Отдельных выпадающих фильтров страны и населённого пункта нет: они заменены общим поиском решением владельца от 22 августа 2026 года. Страна и населённый пункт остаются полями данных храма (§21) и участвуют в поиске, сортировке и адресной модели.
...
На телефоне по умолчанию одновременно видны карта и список: карта занимает верхнюю полосу высотой 300 px, а список наезжает на неё основным рабочим листом со скруглением верхних углов. Над картой плавают только общий поиск и **«Рядом со мной»**; они сохраняются при работе со списком и занимают минимум места, чтобы карта оставалась видимой. Отдельного переключателя **«Список / Карта»** нет; карта разворачивается на весь экран круглой иконочной кнопкой.
...
- полный адрес храма;
- строку полного адреса с иконкой места и шевроном; сам адрес является действием и открывает отдельный экран карты, где доступно построение маршрута. Видимой подписи действия рядом с адресом нет; для вспомогательных технологий строка получает понятное имя действия (решение владельца от 22 августа 2026 года);
- необязательные описание, контакты храма и фотографию, собранные в свёрнутом по умолчанию блоке **«О храме»** над расписанием;
- дату последнего обновления расписания;
- расписание — один хронологический список ближайших служб;
- транспортную доску;
...
**Публичная страница показывает одно расписание.** Раздел называется **«Расписание»** и содержит один хронологический список конкретных предстоящих служб, отсортированный по дате и времени. Отдельного блока ближайшей службы, отдельного раздела других служб и отдельного публичного раздела недельного шаблона на странице нет: способ хранения и ввода расписания не выносится в публичные категории (решение владельца от 22 августа 2026 года).

По умолчанию видны **три ближайшие службы**; остальные раскрываются тихим действием на той же странице. Отдельная страница расписания для этого не нужна.

Служба, показанная в списке, сохраняет свои состояния: **отменённая служба** и **изменённое время** видны текстом рядом с датой (§45.3).

Регулярная и разовая службы попадают в один и тот же список; отдельная пометка регулярности в публичном списке не показывается.

Модель расписания при этом не упрощается. Администратору по-прежнему доступны регулярные службы, разовые службы, исключения, отмены и изменения даты или времени (`PS §7.1`, §21); публичные названия категорий расписания (§45.3) остаются терминами модели и экранов управления храмом.

Рядом с расписанием показывается ненавязчивое предупреждение, что расписание могло измениться. Если нужной службы нет, человек всё равно может создать поездку на собственные дату и время.
...
По умолчанию показаны обе категории. Фильтр называет **роль человека**, а не наличие места: пассажир сразу переходит к водителям, водитель — к пассажирам (решение владельца от 22 августа 2026 года; заменяет прежние **«Есть места»** и **«Ищут место»**). Метка типа на карточке использует то же слово в единственном числе — **«Водитель»** и **«Пассажир»**. Множественное число фильтра и единственное число метки — разные грамматические роли одного термина, а не две формулировки одной роли.

Предложения и просьбы различаются текстом, содержанием, формой маркера и основным действием; цвет служит только дополнительным признаком.

На телефоне карточки находятся в одном потоке. На компьютере используются две колонки **«Водители»** и **«Пассажиры»**; заголовок колонки заменяет метку типа на карточке. Порядок чтения и работа с клавиатуры должны оставаться понятными.

Подзаголовка под заголовком доски нет: контекст страницы и фильтры сообщают то же самое.

Действие карточки находится у левого края содержимого, после типа, даты и времени, числа мест или пассажиров и сведений о месте. Оно остаётся вторичным.

### 9.2.1. Подходящие поездки

Обычная доска остаётся основным представлением и продолжает показывать все активные объявления. Качественный подбор — только подсказка.

**Метка на карточке.** Карточка, подходящая текущему пользователю по правилам `PS §12`, получает простую метку **«Подходит»**. Метка вторична по отношению к типу карточки и её основному действию. Процентов, баллов, рейтингов и слова «лучший» нет.

**Отдельное представление.** Пользователю доступно представление **«Подходящие мне»** внутри той же доски. Оно фильтрует уже видимые объявления и не является единственным доступным содержимым: обычная доска остаётся открытой всегда.

**Порядок.** В представлении «Подходящие мне» объявления упорядочены прежде всего по наименьшему дополнительному заезду. Примерные дополнительные минуты используются только как вторичный признак при равенстве.

**Объяснение.** Вместо оценки показываются конкретные сведения: число свободных мест, подходящее место встречи и короткая строка вида **«+4 км · примерно +8 мин»**. Если пассажиру подходят несколько его мест, показываются все подходящие варианты и отмечается лучший; остальные не скрываются.

**Подбор ничего не решает.** Метка не создаёт договорённость, не отправляет отклик и не запрещает договариваться вручную с тем, кого приложение подходящим не назвало.

**Когда проверка недоступна.** Если проверить поездки временно невозможно, объявление не помечается как подходящее и не помечается как неподходящее. На обычных карточках технических сообщений нет. Только когда человек сам открыл «Подходящие мне», показывается короткая понятная строка: **«Не удалось проверить подходящие поездки. Посмотрите все объявления.»** Названия поставщиков, интерфейсов и технические причины пользователю не показываются.

**Изменение исходных данных.** Неподтверждённый результат подбора производный: при изменении мест, времени, числа людей, условий о детях, допустимого отклонения или состояния объявления он пересчитывается или перестаёт показываться. Подтверждённая договорённость от этого не отменяется: её судьбой управляют правила договорённостей и отмены.

### 9.3. Карта поездок группы

Транспортная доска остаётся представлением по умолчанию. Карта — отдельное пространственное представление той же группы, помогающее сориентироваться тем, кто плохо знает район. Она не заменяет доску и не заменяет качественный подбор поездок (§9.2.1).

**Принадлежность группе.** Карта открывается для одной конкретной группы доски — одной службы или одной собственной даты — и показывает только активные объявления этой группы. Поездки других служб и дат на неё не попадают. Служба или дата видна в заголовке карты.

**Вход на карту.** Действие входа принадлежит группе и находится вне группы фильтров **«Все / Водители / Пассажиры»**:

- на телефоне — **«Поездки на карте»** вне потока карточек, у нижнего края содержимого доски и выше нижней навигации; под него резервируется место, поэтому оно не перекрывает карточки, блок «Уже договорились» и навигацию;
- на компьютере — **«На карте»** в заголовке группы службы или даты.

Действие не является главным: обе формулировки слабее пары **«Нужна поездка / Могу подвезти»**.

**Когда входа нет.** Если в группе нет активных поездок, действия входа не существует. Если активный фильтр не содержит ни одного объекта карты, пустая карта не предлагается: соответствующий фильтр недоступен. **Поправка 3 сентября 2026 года:** видимой причины рядом с недоступным фильтром нет — приглушённый и неактивный фильтр сообщает это сам; причина остаётся доступной вспомогательным технологиям и видимым текстом не становится.

**Фильтры.** Карта использует те же фильтры. Активный фильтр доски переносится на карту, смена фильтра на карте меняет состав видимых объектов, а возврат приводит на доску той же группы с тем же фильтром.

**Что показывается.** Публично видны только приблизительные представления, правила которых заданы в §6:

- просьба пассажира — примерная область радиусом 1 км (§6.2); у одной просьбы может быть до трёх допустимых мест встречи, и тогда показываются все соответствующие области;
- предложение водителя — примерная область отправления (§6.3);
- точный адрес храма, поскольку он публичен.

Линий маршрута и коридоров направления на карте нет.

Типы различаются подписью и оформлением области, а не только цветом. У области нет центральной отметки, чтобы публичный центр не воспринимался как точное место.

**Выбор объекта.** Выбор области открывает карточку соответствующей поездки: на телефоне — листом снизу, на компьютере — панелью рядом с картой. Карточка сохраняет тот же состав сведений и то же основное действие, что и на доске; после явного выбора одной поездки её действие становится главным. Если просьбе принадлежат несколько областей, выбор любой из них открывает одну и ту же карточку, а все её области получают состояние выбранного объекта; линиями они не соединяются. Закрытие карточки возвращает к карте, возврат с карты — к доске.

**Поправка 3 сентября 2026 года.** Две уточняющие правки, принятые владельцем на поэкранной проверке копии:

- **Просьба с несколькими областями не называет место.** Пока место встречи не выбрано, карточка на карте не повторяет строку доски «Место встречи: {место}» — она считает публичные альтернативы («2 возможных места встречи»). У просьбы с одной публичной областью строка доски сохраняется. Точные места не показываются ни в одном случае.
- **Открытая карточка не закрывает выбранное.** Когда карточка открывается, карта смещается — и при необходимости отдаляется — так, чтобы все области выбранной поездки оставались в свободной части карты: над листом на телефоне, слева от панели на компьютере. Это перемещение существующего изображения, а не новое поведение карты: маршруты, поиск и дополнительные данные при этом не появляются.
- **Легенда — в левом нижнем углу карты,** с постоянным отступом от левого и нижнего краёв. Когда на телефоне открыт лист поездки, она держит тот же отступ над листом, а не уходит под него; на компьютере она остаётся в левом нижнем углу, пока карточка занимает правый край. Свободная часть карты считается с учётом легенды, поэтому выбранная область не может оказаться под ней.

**Местоположение пользователя.** Геолокация никогда не запрашивается автоматически. На карте есть явное действие **«Показать, где я»**; ранее полученное в текущем сеансе местоположение может использоваться повторно без нового запроса.

**Примерное расстояние.** Когда местоположение доступно, карточка может показывать короткое округлённое значение вида **«≈ 6 км от вас»**. Оно рассчитывается только до публичной примерной области и никогда до скрытого точного места. Если местоположение недоступно, строка не показывается.

### 9.4. «Уже договорились»
...
Форма состоит из четырёх смысловых экранов. Каждый экран отвечает на один вопрос человека; близкие поля одного и того же вопроса не разносятся по разным экранам (§28.3).

### 10.1. Экраны формы

**Экран 1 — когда.**

- выбрать предстоящую службу храма;
- либо указать собственные дату и время прибытия.

Это один вопрос: служба и собственные дата со временем — два способа на него ответить.

**Экран 2 — где забрать.**

- указать одно обязательное точное место встречи;
- при желании добавить ещё одно или два возможных места (§10.2);
- здесь же человек видит объяснение публичности места (§10.4).

Карта — **часть этого экрана и видна на нём всегда** (поправка от 26 августа 2026 года). Отдельного экрана карты нет, и последовательности «пусто → карта → выбранное место» тоже нет: человек одновременно видит карту, отмеченный адрес, уже сохранённые места и способ сохранить ещё одно. Никуда не переходят и ниоткуда не возвращаются.

На экране по порядку: вопрос, объяснение публичности, карта с полем адреса, отмеченный адрес, действие, сохраняющее отмеченную точку, список сохранённых мест и «Далее».

- отмеченную точку сохраняет **«Добавить место»**; при изменении уже сохранённого места то же действие называется **«Подтвердить место»** — это разные намерения человека, а не переименование одного;
- **«Изменить это место»** переводит сохранённое место на карту и подсвечивает его карточку, не уводя с экрана;
- после сохранения маркер переходит на ещё свободный адрес, чтобы карта была готова к следующему варианту;
- пока сохранённого места нет, «Далее» не показывается: одно место обязательно, а неактивной кнопки без причины не остаётся (§28.3).

Выбранный адрес показывается **один раз**. Отдельного поля для названия места форма не предлагает: ориентир вроде «у входа в библиотеку» человек пишет в обычном необязательном примечании на экране 4 (§10.1, экран 4). Второе поле, похожее на адрес, рядом с адресом только запутывает.

**Экран 3 — кто едет.**

- общее число пассажиров, включая детей;
- число детей;
- нужно ли детское кресло.

**Экран 4 — последние детали.**

- нужна ли поездка обратно;
- короткое примечание, необязательно; здесь же человек уточняет ориентир места встречи;
- компактное резюме введённых сведений;
- действие **«Опубликовать»**.

Компактное резюме — это несколько строк на том же экране рядом с кнопкой, а **не отдельный экран проверки**. Отдельного экрана проверки перед публикацией нет и он не возвращается позже ни под каким названием: ни «Проверьте просьбу», ни «Что увидят все», ни «Что увидит водитель». Публичное и закрытое представление просьбы не показывается отдельным обязательным шагом.

Резюме содержит ровно то, что человек ответил на предыдущих трёх экранах:

- когда — выбранная служба либо собственные дата и время;
- место встречи; если мест несколько, они перечисляются как альтернативы, тем же словом «или», что и на экране 2, — не как остановки;
- число пассажиров и число детей;
- потребность в детском кресле — только когда в группе есть дети (§47.6).

Резюме стоит **над** главным действием, а не под ним, и не имеет заголовка: заголовок над ним вернул бы снятый экран проверки под другим названием. У каждой строки — тихое **«Изменить»**, которое ведёт на экран, которому принадлежит ответ; возврат оттуда сохраняет остальную форму (§28.3). Примечание в резюме не повторяется: оно стоит на том же экране прямо над ним.

Нажатие **«Опубликовать»** — это намерение опубликовать. Если аккаунт, подтверждение email, подтверждение телефона или принятие Условий ещё не выполнены, в этот момент начинается контекстная регистрация и проверка контактов (§19), после которых человек возвращается к заполненной форме и публикует её сам. Это условие публикации, а не пятый смысловой экран формы.
...
Что это варианты, а не остановки, говорят подписи мест («Основное место встречи», «Ещё одно место встречи») и слово «или» между карточками. Отдельного объясняющего абзаца рядом с ними нет: при трёх сохранённых местах на экране остаётся одна короткая причина, почему больше добавить нельзя, и второго текста о пределе рядом не показывается (поправка от 26 августа 2026 года).
...
- Место выбирается на карте, которая постоянно видна на экране «Где вас забрать?» (§10.1, поправка от 26 августа 2026 года).
- Отдельного поля публичной области нет: человек указывает только реальное место, а публичное представление создаёт система.
- Объяснение публичности места стоит на экране выбора места, до подтверждения места, а не отдельным шагом перед публикацией (§10.4).
- Отдельный обязательный экран итога перед публикацией не показывается.
...
### 10.4. Объяснение публичности места

Объяснение находится там, где человек принимает решение, — на экране выбора места встречи и рядом с уже выбранными местами. Отдельного предупреждающего шага перед публикацией нет.

Объяснение не утверждает, что точный адрес увидят все.

Текст (утверждён владельцем 24 августа 2026 года на поэкранной проверке группы 2A):
...
- объяснение видно до подтверждения места и остаётся рядом с выбранными местами, пока они видны;
- одно правило — одна формулировка на экране: вторая формулировка того же правила рядом не показывается;
- в малонаселённой местности дополнительно показывается предупреждение §6.2 о том, что круг радиусом 1 км может косвенно указать на отдельный дом; это другое сообщение и другое условие;
- у поля примечания стоит собственное короткое предупреждение не писать домашний адрес, телефон, email и ссылки (§6.4);
...
Форма состоит из четырёх смысловых экранов. Каждый экран отвечает на один вопрос человека; близкие поля одного и того же вопроса не разносятся по разным экранам (§28.3). Прежняя последовательность из четырнадцати шагов не используется (решение владельца от 27 августа 2026 года).

### 11.1. Экраны формы

**Экран 1 — когда.**

- вверху экрана — разовая или регулярная поездка;
- для разовой поездки: выбрать предстоящую службу храма либо указать собственные дату и время прибытия;
- для регулярной поездки: выбрать службу либо указать собственное время прибытия, затем дни недели и период серии.

Это один вопрос. Как часто человек ездит — не отдельное решение и не отдельный экран: это первая половина ответа на вопрос «когда», от которой зависит вторая. Дни недели и период — часть того же ответа и остаются на том же экране.

У регулярной поездки нет одной даты, поэтому службы на ней называются недельным временем («по воскресеньям, 9:00»), а не ближайшей датой, и собственный ответ спрашивает время, а не дату со временем: даты серии дают дни недели и период.

Человек указывает время **прибытия к храму**. Другого времени форма не спрашивает: сопоставление сравнивает указанные времена напрямую (§11.7, `PS §12.2`).

**Экран 2 — откуда.**

- точное место отправления;
- дорога до храма, нарисованная на карте, — временная подсказка автору, помогающая понять выбранное место (§6.3);
- допустимое отклонение от маршрута в километрах;
- здесь же человек видит объяснение публичности места (§11.6).

Карта — **часть этого экрана и видна на нём всегда**. Отдельного экрана подтверждения места и отдельного экрана построения маршрута нет: место и допустимое отклонение — один вопрос «откуда вы едете», разложенный ранее на три экрана.

Нарисованная на карте дорога — **временная подсказка автору внутри формы** (§6.3). Она видна только самому водителю, пока он создаёт или изменяет предложение; она не входит в опубликованное предложение, никому больше не показывается, не сохраняется как маршрут водителя, и водитель не обязуется ехать именно по ней. Отдельного пояснения рядом с ней в интерфейсе нет.

У экрана два состояния одного и того же вопроса.

**Пока место не выбрано** на экране стоят: карта, поле адреса, короткая инструкция по выбору места, одно тихое предложение о публичности (§11.6) и одно действие — **«Подтвердить место»**. Больше на экране ничего нет: всё остальное зависит от места. «Далее» не показывается, потому что место обязательно, а неактивной кнопки без причины не остаётся (§28.3).

**После выбора места** поле адреса и инструкция убираются — они сделали свою работу. Выбранный адрес показывается **один раз**, отдельной подписи над ним нет: её заменяет вопрос экрана. На карте появляется маршрут, под адресом — компактный выбор допустимого отклонения и **«Далее»**. Расстояние и продолжительность поездки не показываются: сопоставление поездок их не использует, и решения человека от них не зависят (правка владельца от 29 августа 2026 года).

Рядом с адресом стоит тихое действие **«Изменить место»**: оно возвращает этот же экран в состояние выбора места, никуда не уводя.

Время выезда на этом экране не спрашивается (§11.7).

**Экран 3 — детали поездки.**

- число свободных мест;
- может ли водитель перевозить детей;
- есть ли у него подходящее детское кресло;
- может ли водитель подвезти обратно;
- короткое примечание, необязательно.

Это все оставшиеся изменяемые сведения о поездке, собранные на одном экране. Вопрос о кресле показывается только тогда, когда водитель может везти детей: иначе ответа на него не существует (§2.3, §47.7).

Предупреждение о том, чего не писать в примечании, стоит **подсказкой внутри самого поля**, где человек собирается писать, а не отдельным текстом под ним (§6.4).

**Экран 4 — проверьте поездку.**

Экран **только показывает**. Новых вопросов на нём не задаётся и новых полей на нём нет.

- компактное резюме введённых сведений: **когда**, **откуда**, **места и дети**, **обратно**, **примечание**;
- действие **«Опубликовать»**.

Резюме содержит ровно то, что человек ответил на предыдущих трёх экранах:

- когда — служба либо собственное время; для регулярной поездки также дни недели и период. Правило о том, что каждая дата остаётся отдельной поездкой с отдельными местами (§11.3), здесь не повторяется: оно сказано на экране 1, где выбирается серия;
- откуда — место отправления и допустимое отклонение;
- места и дети — число мест, возможность везти детей и наличие кресла, когда оно есть;
- обратно — может ли водитель подвезти обратно;
- примечание — то, что человек написал, либо обычное пустое значение, если он ничего не писал.

У каждой строки — тихое **«Изменить»**, которое ведёт на экран, которому принадлежит ответ; возврат оттуда сохраняет остальную форму (§28.3).

Публичного представления поездки на этом экране **нет**: ни картинки примерной области и примерного направления, ни блоков «Так вашу поездку увидят все» и «Так её увидит пассажир после договорённости». Объяснение публичности здесь **не повторяется** — оно сказано один раз на экране 2 (§11.6).
...
### 11.6. Объяснение публичности места отправления

Публично место отправления водителя показывается только примерной областью по правилам §6.2. Публичного маршрута водителя не существует (§6.3): ни линии улиц, ни коридора направления, ни какой-либо публичной геометрии поездки. Точное место отправления открывается пассажиру только после взаимного подтверждения.

Объяснение этого правила находится там, где человек принимает решение, — на экране «откуда», в состоянии выбора места (§11.1, решения владельца от 27 и 28 августа 2026 года). Отдельного предупреждающего шага перед публикацией нет.

Это **одно тихое предложение**, а не выделенная панель: короткое правило, оформленное как предупреждение, читается как сообщение о том, что что-то пошло не так.

Текст (`Проект`, ждёт поэкранной проверки):

> Всем будет видна только примерная область отправления — точный адрес откроется после договорённости.

Правила:

- объяснение видно до выбора места и уходит вместе с состоянием выбора: после выбора места решение уже принято, а повторять правило нечем;
- одно правило — одна формулировка на всю форму: вторая формулировка того же правила не показывается ни на этом экране, ни на следующих;
- на экране 4 публичного представления поездки **нет**: ни картинки, ни блоков «Так вашу поездку увидят все» и «Так её увидит пассажир после договорённости»;
- в малонаселённой местности дополнительно показывается предупреждение §6.2 о том, что круг радиусом 1 км может косвенно указать на отдельный дом; это другое сообщение и другое условие;
- у поля примечания стоит собственное короткое предупреждение не писать цену, телефон, email, ссылки и точный адрес; оно стоит подсказкой внутри поля (§6.4, §11.1, экран 3);
- второго подтверждения приватности перед публикацией не создаётся.

### 11.7. Время поездки

Человек отвечает на вопрос «когда» временем **прибытия к храму**: он едет к службе, и служба назначена на своё время (§11.1, экран 1). Другого времени форма не спрашивает.

Отдельного поля времени выезда в форме нет и связанных с ним пояснений тоже (решение владельца от 28 августа 2026 года). Временная проверка сравнивает **указанные** времена напрямую: службу с той же службой, а собственное время — с желаемым временем пассажира, без поправки на заезд (`PS §12.2`). Время водителя — цель, которую он на себя берёт: если заезд за пассажиром требует лишних минут, водитель выезжает раньше, а не приезжает позже. Значения, которое пришлось бы для этого собирать, у формы нет.

Продолжительность поездки по той же причине на экране «откуда» не показывается (правка владельца от 29 августа 2026 года). Примерные дополнительные минуты остаются языком подсказки о подходящей поездке (§12), а не полем формы.

Требование `PS §9.1` о «примерном времени выезда» снято при синхронизации 2 сентября 2026 года вместе с вливанием кампании Maps & Quality Matching: правило, ради которого это значение собиралось, отменено самой кампанией.
...
- одна и та же служба либо совместимые собственные дата и время: указанное водителем время прибытия к храму не раньше чем на час и не позже чем на полчаса от желаемого времени пассажира, включительно (поправка от 28 августа 2026 года);
...
- требования по перевозке детей совместимы: при детях в группе водитель должен возить детей, а при необходимости детского кресла — иметь его;
- хотя бы одно место пассажира укладывается в допустимое отклонение водителя по дополнительным километрам.

Если мест меньше, чем людей в группе, автоматическое совпадение не предлагается. Частичная договорённость по-прежнему возможна вручную через обычную доску (§13.3 `PS`).

Отдельного условия «совпадение направления» нет. Достаточно того, что заезд укладывается в допустимое отклонение.

Система использует точные координаты для расчёта совместимости на сервере, но не раскрывает их до договорённости. Рассчитанная дорога используется только для получения дополнительных километров и примерных минут; она не сохраняется как маршрут поездки и не публикуется.
...
- роль пользователя;
- выбранное точное место встречи;
- точное место отправления водителя;
...
- доступ к контактам, точному месту встречи и точному месту отправления закрывается сразу;
...
- старая публичная ссылка не показывает место и примечание;
...
Раздел **«Мои объявления»** назван так, чтобы внутри «Моих поездок» было сразу понятно: это собственные просьбы и предложения пользователя, а не общий каталог объявлений (решение владельца от 4 сентября 2026 года).

**Карточка не повторяет раздел.** Открытая вкладка **«Требуют ответа»** уже сообщает, что от пользователя ждут решения, поэтому карточка в ней не носит ни плашки состояния, ни фразы об этом. Она показывает только то, о чём решение: роль, службу с датой и временем, город, второго участника с его ролью, условия его словами и два действия.

**Имя второго участника никогда не показывается без роли:** «Водитель: Алексей», «Пассажир: Мария». Исключение — список под заголовком, который роль уже называет.

В разделах, которые состояние сами не сообщают, состояние остаётся **короткой плашкой** и не превращается в пересказ произошедшего. Родовые формы в таких строках не используются: грамматический род второго участника сервису неизвестен, и данных о нём не хранится.

**«Мои объявления» делятся по тому, что пользователь опубликовал:**

- **Ищу поездку** — собственные просьбы;
- **Могу подвезти** — собственные предложения.

Один аккаунт может иметь обе группы: роли — это действия, а не режимы (§2.2), и переключателя «водитель / пассажир» это не создаёт.

Внутри каждой группы порядок карточек — по тому, насколько объявление нуждается в пользователе:

1. требует его ответа;
2. ждёт ответа второй стороны;
3. просто опубликовано.

Эти три состояния сообщаются плашкой на карточке — **«Нужен ваш ответ»**, **«Ждём ответа»**, **«Активно»** — и отдельными подзаголовками не становятся.

Карточка с «Нужен ваш ответ» ведёт действием **«Ответить»**; остальные — «Открыть». Открытое объявление, которое ждёт ответа, показывает обращение второго участника первым — с его ролью, условиями и действиями «Подтвердить поездку» / «Отказаться», — а ниже отдельным блоком управление самим объявлением. Ответ человеку и управление объявлением не смешиваются. Тот же ответ доступен и из раздела «Требуют ответа»: это один отклик, показанный в двух контекстах, а не две записи.

### 16.1. Пустые состояния

- Если во всех четырёх разделах ничего нет, показывается общий экран с действием **«Найти храм»**.
- Если данные есть в других разделах, но выбранный раздел пуст, показывается текст именно для него **без действия**: вкладки над списком и есть навигация, а четыре разных призыва только добавляли шум (решение владельца от 4 сентября 2026 года, заменившее прежнее требование подходящего действия у каждого раздела).
- **«Найти храм»** остаётся действием только совсем пустого раздела «Мои поездки».
- Компоновка пустых экранов может быть общей, но текст каждого раздела свой.
...
**Список показывает объявление, а не все его поля.** Карточка в списке называет, что это, кратко и в одну строку — ритм и число ближайших дат для регулярной поездки, число людей или свободных мест для разовой, — и открывается. Условия поездки, сведения о детях, обратная поездка и отдельные даты регулярной поездки показываются **внутри открытого объявления**, а не в общем списке. Таблица дат в основном списке не показывается.

**Управление собственным объявлением находится внутри него.** В списке нет действий редактирования у каждой карточки; вход один — открыть объявление (или ответить, если оно ждёт ответа). Внутри отдельным блоком доступны **«Изменить объявление»** и **«Снять объявление»**.

**Снять объявление и отозвать обращение — разные действия.** Снятие убирает с доски собственное опубликованное объявление. Отзыв (§12.6) отменяет отдельное обращение к конкретному человеку и объявление на доске не трогает. Одно действие не называется словами другого.

Конкретная подтверждённая дата регулярной поездки может быть видна и как поездка в разделе «Предстоящие», и как одна из дат внутри открытого объявления. Это разные контексты, а не дублирование одной записи.
...
**Открытая договорённость разбита на смысловые группы**, а не на один сплошной столбец фактов: где и когда встреча — отдельно, сведения о самой поездке — отдельно, контакты — отдельным блоком. Состояние сообщается один раз: если плашка говорит «Подтверждено», предложение того же смысла рядом не пишется.

**Главное действие подтверждённой предстоящей поездки — связаться со вторым участником.** Оно оформляется как единственное акцентное действие экрана внутри блока контактов. Изменение условий и отмена стоят ниже отдельным блоком **«Управление поездкой»**; жалоба — тихое текстовое действие под всем. Главными действиями они не становятся.

**История поездки до самой поездки не показывается.** У предстоящей договорённости входа в её историю нет — ни у пассажира, ни у водителя.

**Подтверждённые участники и ожидаемые ответы различимы.** Там, где ответа ждут не от пользователя, показывается, что уже произошло, и действие не предлагается: отвечать должен второй человек. Действие, которого ждут от самого пользователя, находится в разделе «Требуют ответа».
...
Контакты и открытые по договорённости точные места остаются доступными участникам не более 30 дней после запланированного времени: водителю — выбранное точное место встречи, пассажиру — точное место отправления водителя. Затем они скрываются, а точные геоданные удаляются или необратимо обезличиваются. В истории остаются храм, дата, роль, число людей, итог и примерная область без возможности восстановить точку.

Маршрут в этот срок не входит и вообще не хранится: он рассчитывается только на время проверки «по пути» (§6.3), а сохраняются лишь дополнительные километры и примерные минуты. Сохранённые пользователем места живут по собственному правилу (§6.7) и этим сроком не ограничены.
...
9. Возвращается к сохранённому действию — к заполненной форме или к итогу того действия, у которого он есть.
10. Сам нажимает **«Опубликовать»** или **«Отправить»**: автоматически ничего не публикуется.
...
- На новом устройстве пользователь после проверок видит сохранённое действие и завершает его сам.
...
- точные координаты;
- рассчитанная геометрия маршрута;
- телефон и email;
- имя;
...
- **Требуют ответа:** «Сейчас нет поездок, которые требуют вашего ответа.»;
- **Предстоящие:** «У вас нет предстоящих договорённостей»;
- **Мои объявления:** «У вас нет активных просьб и предложений»;
- **История:** «Завершённых и отменённых поездок пока нет».

Действие в этом состоянии не показывается (§16.1).
...
- Поля, которые вместе отвечают на этот вопрос, остаются на одном шаге. Правило одного вопроса запрещает смешивать разные вопросы, а не разделяет близкие поля: «сколько вас будет» — это общее число пассажиров, число детей и потребность в детском кресле на одном экране (§10.1).
...
- Перед действительно необратимым действием — удалением аккаунта, отменой договорённости — показывается проверка. Публикация объявления к таким действиям не относится: объявление можно изменить и отменить, поэтому обязательный экран проверки перед ним не создаётся (§10.1).
...
- Телефон: карта высотой 300 px сверху; основной лист списка со скруглёнными верхними углами наезжает на карту; общий поиск и «Рядом со мной» плавают над картой; круглое иконочное действие открывает полноэкранную карту. Отдельного переключателя списка и карты нет.
- Компьютер: список слева, карта справа.
- Выбор храма и результаты поиска синхронизированы между списком и картой.
...
- Отдельной компактной карты на странице нет; сам адрес является действием и открывает экран карты храма.
...
Просьба пассажира — четыре смысловых экрана формы (§10.1):

- когда: выбор службы или собственных даты и времени;
- где забрать: выбор первого места на карте, добавление до двух альтернатив, объяснение публичности места на том же экране;
- кто едет: общее число пассажиров, число детей, детское кресло;
- последние детали: поездка обратно, примечание, «Опубликовать».

Предложение водителя — четыре смысловых экрана формы (§11.1):

- когда: разовая или регулярная поездка, служба или собственное время прибытия, для регулярной — дни недели и период;
- откуда: два состояния одного экрана — выбор места на карте с объяснением публичности, затем выбранный адрес, маршрут, допустимое отклонение и «Изменить место»;
- детали поездки: число мест, дети, детское кресло, поездка обратно, примечание;
- проверьте поездку: только резюме из пяти строк с тихим «Изменить» и «Опубликовать».

Общие состояния обеих форм:
...
3. Отвечает на четыре вопроса формы: когда, где забрать, кто едет, последние детали (§10.1).
4. На экране места видит, что публично будет видна только примерная область.
5. Нажимает «Опубликовать».
6. Вводит имя, email и телефон.
7. Принимает Условия и заявление 18+.
8. Переходит по ссылке из email.
9. Вводит SMS-код.
10. Возвращается к заполненной форме и публикует её сам.
...
3. Отвечает, когда едет: разовая поездка или серия до 8 недель, служба или собственное время (§11.1, экран 1).
4. Отвечает, откуда: выбирает место отправления на карте и читает, что публично видна только примерная область; после выбора видит адрес, маршрут, дорогу и допустимое отклонение (§11.1, экран 2; §11.6).
5. Указывает детали поездки: число мест, возможность везти детей и наличие кресла, поездку обратно и при желании примечание (§11.1, экран 3).
6. На последнем экране проверяет собранное резюме и при необходимости возвращается к нужному экрану тихим «Изменить» (§11.1, экран 4).
7. Входит или подтверждает контакты — условие публикации, а не отдельный экран формы (§19).
8. Публикует.
9. Получает и рассматривает обращения по каждой дате.
...
6. Открываются выбранное точное место встречи, точное место отправления водителя, телефон и email.
...
| Точные места подтверждённой поездки | Доступ участникам до 30 дней после времени поездки; затем удалить или необратимо обезличить. Сохранённые места пользователя живут по собственному правилу (§6.7) |
...
- [ ] Публичная область отправления не позволяет восстановить точное место отправления, а публичной линии маршрута не существует.
...
- точный адрес виден до договорённости;
...
6. Один подтверждённый номер нельзя одновременно привязать к двум аккаунтам; простой ввод номера без SMS-подтверждения не создаёт привязку.
...
- алгоритм построения примерной области;
...
- поисковая строка каталога без личных данных (общий поиск заменил отдельные параметры страны и населённого пункта, §8.1);
- вид `list` или `map`;
- фильтр **«Все / Водители / Пассажиры»**;
...
- телефон, email и имя;
- точное место встречи и точное место отправления;
- текст примечания;
...
Категории описывают модель расписания и экраны управления храмом. На публичной странице храма они не показываются как отдельные разделы: там одно хронологическое расписание (§9.1).

- **Ближайшие службы** — вычисленный список конкретных дат; на публичной странице это и есть содержимое раздела «Расписание».
- **Регулярное расписание** — повторяющийся недельный шаблон; название раздела управления храмом.
...
Состояния **«Служба отменена»** и **«Время изменено»** видны и на публичной странице рядом с соответствующей датой.
...
| Мои объявления | Активные просьбы и предложения автора, включая регулярную поездку; делятся на «Ищу поездку» и «Могу подвезти», внутри группы — «Нужен ваш ответ», «Ждём ответа», «Активно» |
...
Наличие аккаунта само по себе не открывает более точную географию. Анонимный посетитель и вошедший пользователь, не участвующий в конкретной договорённости, видят одно и то же приблизительное представление. Более точные сведения открывает только участие в подтверждённой договорённости.
...
| Телефон | Да для действий | Пользователь | `ACCOUNT`, затем `PARTICIPANTS` | Международный формат; принадлежность и уникальность возникают после подтверждения, дата которого хранится отдельно |
...
| Подпись места | Нет | Автор, затем `PARTICIPANTS` | Форма просьбы пассажира первой версии это поле не предлагает (§10.1, экран 2); ориентир пишется в обычном примечании. Проверяется на контакты |
...
| Точное отправление | Да | Закрыто | Имеет производную примерную область; открывается пассажиру после подтверждения |
...
| Подтверждённый снимок условий | Да | `PARTICIPANTS` | Дата, время, выбранное место, люди, дети, возврат |
...
stateDiagram-v2
    ...
    Verification --> Draft: проверка завершена, вернуть к заполненной форме
...
- понятно ли различие **«Мои объявления»** и **«Предстоящие»**;
