...
`test:staging-core` is a separately guarded remote smoke command for an already linked, empty project named exactly `orthodox-routes-staging`. It retrieves project keys only through the authenticated Supabase CLI, exercises remote Auth/Data API coordination with synthetic identities, and removes its fixtures. It does not configure or verify real email or SMS delivery and must never target production.
