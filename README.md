...
- a server-owned Core transport foundation for eligible passenger requests, one-time driver offers, bounded regular series, date-specific occurrences, owner-only exact labels, anonymous safe projections, idempotent publication/cancellation, and database-enforced ownership.
...
The first server-owned transport slice now derives every mutation actor from Auth, rechecks authoritative participation eligibility, and stores passenger requests, driver series, and date-specific occurrences in PostgreSQL. Direct table access remains revoked; anonymous clients receive only deliberate safe projections, while exact pre-map labels are returned only to their owner. The Core migration deliberately does not add coordinates, routes, PostGIS, or matching: those remain in the Maps phase. No production Terms document, SMS provider, remote Supabase project, or transport application cutover is configured, so the current production UI still exercises its isolated browser mock flow until the later campaign integration checkpoint. Browser records are not imported or trusted by the server domain.
...
GitHub Actions runs on pushes to `main` and pull requests targeting `main`. The frontend job preserves the clean-install tests, lint, and production build. A separate database-foundation job starts the local database and runs `db:reset`, proving that all committed migrations replay from a clean database without remote projects or secrets. A separate local-auth job starts the committed local Auth/Data API configuration, runs the focused Auth, account/eligibility, phone-verification, contextual-draft, and server-owned transport boundary tests, and always stops the stack; it requires no Supabase account, access token, remote project, SMTP/SMS credentials, or repository secrets.
