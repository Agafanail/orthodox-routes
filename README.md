...
GitHub Actions runs on pushes to `main` and pull requests targeting `main`. The frontend job preserves the clean-install tests, lint, and production build. A separate database-foundation job starts the local database and runs `db:reset`, proving that all committed migrations replay from a clean database without remote projects or secrets. A separate local-auth job starts the committed local Auth/Data API configuration, runs the focused Auth, account/eligibility, phone-verification, contextual-draft, transport, agreement, concurrency, and disclosure boundary tests, and always stops the stack; it requires no Supabase account, access token, remote project, SMTP/SMS credentials, or repository secrets.
