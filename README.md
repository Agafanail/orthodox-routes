...
The configured Core church board requires a matching published `app.church` row. It uses RPC state exclusively and does not read or write the historical transport `localStorage` keys. Protected contact and exact-place values are fetched only after an explicit participant action and are not included in the ordinary board response. See [Core Multi-User Staging Readiness](docs/CORE_STAGING_READINESS.md) for the exact remote environment, migration, provider, privacy, and smoke-test gates; that document is readiness guidance, not authority to access or mutate a remote project.
