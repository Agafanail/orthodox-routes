...
- participant-authorized agreement change proposals with immutable pending/active snapshots, an authoritative responder, atomic capacity changes on acceptance, and accepted-versus-proposed detail reads;
- a participant-only `api.get_my_trip_details(p_agreement_id)` read for an explicitly opened agreement, with accepted and pending child/seat and return conditions, current remaining passenger need, and the existing bounded contact/exact-place disclosure;
- an always-enabled account-owned notification history, ordinary ride email/Web Push preferences, protected per-device subscriptions, an enforced effective-external-channel rule, and a durable provider-independent delivery outbox with bounded leases and retries;
...
`api.propose_agreement_change(p_agreement_id, p_changes, p_client_key)` creates one immutable pending conditions snapshot while the accepted snapshot and capacity remain active. Only the other participant can call `api.resolve_agreement_change(p_proposal_id, p_accept, p_client_key)`. Declining preserves the accepted snapshot. Accepting replaces it and adjusts request need and occurrence capacity atomically; passenger-count decreases return need and seats, while increases must still fit both. Proposal and decision calls are idempotent, both participants must remain eligible, and cancellation or lifecycle completion closes an unresolved proposal. `api.current_my_trips()` exposes only the proposal ID, roles, deadline, and action flag; it never adds contacts or exact places to the list.

`api.get_my_trip_details(p_agreement_id uuid)` is a separate on-demand read for an agreement's participants. It returns the caller's role, counterparty name/role, church, accepted schedule, confirmed count, approximate meeting-area label, cancellation role, and structured accepted conditions (`children_count`, child-seat requirements/capabilities, both return flags, and detour). During `change_pending`, a separate `change` object provides the proposed snapshot, proposed exact places, authoritative proposer/responder roles, deadline, and caller-specific action flag without replacing the accepted fields. Conditions come from immutable snapshots, never from subsequently edited source listings; `counts.remaining_passengers` is explicitly the current unfilled request need. The `contacts` and accepted `places` objects reuse `get_agreement_contacts` and `get_agreement_exact_place` and their independent visibility checks. An unknown agreement, unrelated caller, or unconfirmed response returns null; anonymous and service-role execution is not granted. Cancellation, archival, or elapsed exact-data retention leaves only a safe summary, with null conditions, change, current remaining need, contacts, and places; the contact deadline can separately close contacts sooner. No read performs lifecycle writes. Never prefetch this RPC for a list or put its response into public HTML, shared caches, notifications, or analytics.

`test:my-trips` also verifies both participants' detail reads, partial groups, children/return conditions, immutable accepted terms after source edits, proposal idempotency and authorization, accept/decline behavior, bidirectional atomic capacity deltas, unrelated/malformed/spoofed access, omission of unused exact places and notes, immediate cancellation, and visibility before and after lifecycle cleanup. Group 5 is approved as an isolated copy-review surface; production My Trips UI is still absent.

`test:notifications` verifies account isolation for the always-enabled in-app history, safe structured parameters, read state, default email fallback, idempotent device subscription storage, preference changes, the active-commitment external-channel invariant, service-role-only delivery leasing, bounded retry state, terminal push invalidation, the in-app channel warning and operational alert, table grants, and forced RLS. Delivery destinations and Web Push key material exist only in protected storage and a channel-filtered service-role worker lease; account history and preferences never return them. `POST /api/internal/notification-worker` requires a constant-time-checked scheduler secret and dispatches through provider-independent Resend and VAPID adapters with durable idempotency. `POST /api/webhooks/resend` verifies the raw Svix signature, retains only minimized 90-day receipts, deduplicates replayed events, and never lets out-of-order events regress a terminal delivery. No provider secret or destination is returned by either endpoint. Transport-event insertion, reminders, owner-controlled provider configuration, and real device/provider verification remain separate campaign work.
