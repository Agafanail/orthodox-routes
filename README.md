...
`test:notifications` verifies account isolation for the always-enabled in-app history, safe structured parameters, read state, default email fallback, idempotent device subscription storage, preference changes, the active-commitment external-channel invariant, service-role-only delivery leasing, bounded retry state, terminal push invalidation, the in-app channel warning and operational alert, table grants, and forced RLS. Delivery destinations and Web Push key material exist only in protected storage and the service-role worker lease; account history and preferences never return them. This foundation does not send provider traffic. Resend and VAPID adapters, signed delivery webhooks, transport-event insertion, and real device/provider verification are separate campaign work.
